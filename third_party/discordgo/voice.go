...
type voiceClientConnect struct {
	UserID    string `json:"user_id"`
...
func (v *VoiceConnection) setSSRCMapping(ssrc uint32, userID string) {
	if ssrc == 0 || userID == "" {
		return
	}
	v.Lock()
...
func (v *VoiceConnection) removeSSRCMapping(ssrc uint32) {
	...
	v.Lock()
	if v.ssrcMap != nil {
		delete(v.ssrcMap, ssrc)
	}
	v.Unlock()
}

func (v *VoiceConnection) userIDForSSRC(ssrc uint32) string {
	v.RLock()
	defer v.RUnlock()
	if v.ssrcMap == nil {
		return ""
	}
	return v.ssrcMap[ssrc]
}
...
func (v *VoiceConnection) onEvent(message []byte) {
	...
	case 5:
		...
		if voiceSpeakingUpdate.SSRC != 0 && voiceSpeakingUpdate.UserID != "" {
			v.setSSRCMapping(uint32(voiceSpeakingUpdate.SSRC), voiceSpeakingUpdate.UserID)
		}
	...
	case 12: // CLIENT_CONNECT
		client := &voiceClientConnect{}
		if err := json.Unmarshal(e.RawData, client); err != nil {
			v.log(LogError, "OP12 unmarshall error, %s, %s", err, string(e.RawData))
			return
		}
		v.log(LogInformational, "voice client connect user=%s audio_ssrc=%d", client.UserID, client.AudioSSRC)
		if client.AudioSSRC != 0 && client.UserID != "" {
			v.setSSRCMapping(client.AudioSSRC, client.UserID)
		}
		return

	case 13: // CLIENT_DISCONNECT
		client := &voiceClientDisconnect{}
		if err := json.Unmarshal(e.RawData, client); err != nil {
			v.log(LogError, "OP13 unmarshall error, %s, %s", err, string(e.RawData))
			return
		}
		v.log(LogInformational, "voice client disconnect user=%s audio_ssrc=%d", client.UserID, client.AudioSSRC)
		if client.AudioSSRC != 0 {
			v.removeSSRCMapping(client.AudioSSRC)
		}
		return
